import { Questionnaire } from "@/lib/types";

/** Humanities AP FRQ-style generated sets paired with /ap/writing-frameworks */
export const humanitiesQuestionnaires: Questionnaire[] = [
  {
    "id": "apush-dbq-practice-a",
    "title": "AP US History — DBQ Practice Set A",
    "subject": "AP US History",
    "kind": "generated",
    "description": "Document-based prompts with half-process scaffolding. Use the DBQ framework on /ap/writing-frameworks. Official exam shape: Section I: 55 stimulus MCQ + 3 SAQs. Section II: 1 DBQ + 1 LEQ.",
    "generationNote": "Original prompts — not College Board exam text.",
    "estimatedMinutes": 45,
    "difficultyTier": 2,
    "tags": [
      "DBQ",
      "writing",
      "humanities"
    ],
    "items": [
      {
        "id": "apush-dbq-a1",
        "format": "frq_half",
        "difficultyTier": 2,
        "conceptId": "ush-periodization",
        "prompt": "Documents (original practice — not College Board):\nDoc 1 (1912, settlement house worker): “Night classes filled once women could vote in school elections.”\nDoc 2 (1911, mill owner): “State factory inspection is an assault on property.”\nDoc 3 (1920, newspaper): “The Nineteenth Amendment closed one fight and opened another over enforcement.”\n\nPrompt: Evaluate the extent to which Progressive Era reforms (1890–1920) expanded democratic participation in the United States.\n\n(a) Write a thesis that responds to the prompt.\n(b) Use TWO documents, explaining how each supports your argument.\n(c) Provide ONE piece of outside evidence.",
        "visibleSteps": [
          "Group documents by argument (political, social, economic).",
          "Draft a thesis with a defensible claim and 2–3 categories."
        ],
        "blankSteps": [
          "Thesis: ________________________________________________",
          "Outside evidence example: ________________________________"
        ],
        "hints": [
          "Qualify one document — explain a limitation or alternate reading.",
          "Use the DBQ framework checklist before writing body ¶1."
        ],
        "examSection": "Section II · DBQ",
        "conceptIntro": "Section II · DBQ"
      }
    ],
    "examFormatNote": "Section I: 55 stimulus MCQ + 3 SAQs. Section II: 1 DBQ + 1 LEQ."
  },
  {
    "id": "apwh-leq-practice-a",
    "title": "AP World History — LEQ Practice Set A",
    "subject": "AP World History",
    "kind": "generated",
    "description": "Long Essay Question practice with comparison/causation frames. Official exam shape: Section I: 55 stimulus MCQ + 3 SAQs. Section II: 1 DBQ + 1 LEQ.",
    "generationNote": "Original prompts for skill practice.",
    "estimatedMinutes": 40,
    "difficultyTier": 2,
    "tags": [
      "LEQ",
      "writing",
      "humanities"
    ],
    "items": [
      {
        "id": "apwh-leq-a1",
        "format": "frq_half",
        "difficultyTier": 2,
        "conceptId": "whap-industrial",
        "prompt": "In the period 1450–1750, compare the effects of maritime exploration on two world regions.\n\n(a) Contextualize the topic (1–2 sentences).\n(b) Write a thesis that takes a position.\n(c) Support with TWO specific examples.",
        "visibleSteps": [
          "Choose two regions and a valid comparison category (economy, state power, culture).",
          "Write a thesis that compares AND explains a reason for difference/similarity."
        ],
        "blankSteps": [
          "Thesis: ________________________________________________",
          "One specific piece of evidence per region: ________________"
        ],
        "hints": [
          "LEQ needs explicit comparison language (similarly, whereas, while).",
          "Contextualization: 1–2 sentences before the thesis."
        ],
        "examSection": "Section II · Long Essay Question",
        "conceptIntro": "Section II · Long Essay Question"
      }
    ],
    "examFormatNote": "Section I: 55 stimulus MCQ + 3 SAQs. Section II: 1 DBQ + 1 LEQ."
  },
  {
    "id": "aplang-rhetoric-practice-a",
    "title": "AP English Language — Rhetorical Analysis Set A",
    "subject": "AP English Language",
    "kind": "generated",
    "description": "Skill practice: rhetorical analysis with SOAPS + thesis scaffolding. For full exam shape (reading/writing MCQ + three FRQs), use AP English Language — Exam Format Set A. Official exam shape: Section I: 45 four-choice MCQ (reading + writing revision, 60 min). Section II: Synthesis, Rhetorical Analysis, Argument (2 h 15 min).",
    "generationNote": "Original rhetorical situations for practice.",
    "estimatedMinutes": 35,
    "difficultyTier": 2,
    "tags": [
      "rhetoric",
      "writing",
      "humanities"
    ],
    "items": [
      {
        "id": "aplang-rhet-a1",
        "format": "frq_half",
        "difficultyTier": 2,
        "conceptId": "elang-rhetoric",
        "prompt": "Excerpt (original practice): “I write not as a contractor but as a parent who walks this corridor at 7:40 a.m. You have heard cost estimates. Hear instead a child on a bike and a paint line that ends.”\n\nExcerpt (original practice): “I write not as a contractor but as a parent who walks this corridor at 7:40 a.m. You have heard cost estimates. Hear instead a child on a bike and a paint line that ends.”\n\nAnalyze how the author of a speech advocating for public parks uses rhetorical choices to build credibility with a city council audience.\n\n(a) Identify the rhetorical situation (speaker, audience, purpose).\n(b) Analyze TWO rhetorical choices and their effects.",
        "visibleSteps": [
          "Identify speaker, occasion, audience, purpose, and tone (SOAPS).",
          "Choose 2–3 rhetorical choices (evidence, structure, style) to analyze."
        ],
        "blankSteps": [
          "Thesis (author + choices + purpose): ______________________",
          "Body ¶1 topic sentence: __________________________________"
        ],
        "hints": [
          "Explain HOW each choice builds credibility — not just what the choice is.",
          "Use the rhetorical analysis framework on /ap/writing-frameworks."
        ],
        "examSection": "Section II · Question 2 Rhetorical Analysis",
        "conceptIntro": "Section II · Question 2 Rhetorical Analysis"
      }
    ],
    "examFormatNote": "Section I: 45 four-choice MCQ (reading + writing revision, 60 min). Section II: Synthesis, Rhetorical Analysis, Argument (2 h 15 min)."
  }
];
