...
export default async function ToolWorkbenchPage({
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-brand-200 bg-brand-50/40 px-5 py-7 sm:px-8">
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-600">
            Modules load only after you choose one. The workbench page does not prefetch every tool in the background.
          </p>
        </div>
          ...
          {modules.map((tool) =>
            tool ? (
              <Link
                ...
                href={tool.href}
                prefetch={false}
                className="group rounded-xl border border-slate-200 bg-white p-4 transition-colors hover:border-brand-300 hover:bg-slate-50 active:bg-slate-100"
              >
