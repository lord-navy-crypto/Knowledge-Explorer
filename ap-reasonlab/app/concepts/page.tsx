...
function ConceptsContent() {
  ...
  return <div className="space-y-6"><div><Link href="/concepts" className="text-sm text-brand-600 hover:underline">← All subject folders</Link><h1 className="mt-2 text-3xl font-bold">{subject}</h1><p className="mt-2 text-slate-600">Use <strong>+ Add topic</strong> to add a topic to this subject. Only this subject’s built-in study data is fetched.</p></div><UnifiedMediaFrame alsoShow={["topic", "concept", "document", "folder"]} defaultSubject={subject} folderArea="concepts" spaceKey={spaceKey} spaceBasePath="/concepts" title={`${subject} · pictures, documents, files & topics`} onSubjectsChange={setManagedSubjects} /><div className="flex flex-wrap gap-2">{([ ["all", "All"], ["concept", "Topics"], ["guide", "Guides"] ] as const).map(([value, label]) => <button key={value} type="button" onClick={() => setFilter(value)} className={filter === value ? "filter-pill-active" : "filter-pill"}>{label}</button>)}</div><input type="text" className="input" placeholder="Search topics in this subject..." value={query} onChange={(e) => setQuery(e.target.value)} />{catalogLoading ? <p className="text-sm text-slate-500">Loading {subject} topics…</p> : <div className="grid gap-3 sm:grid-cols-2">{filtered.length > 0 ? filtered.map((item) => <Link key={`${item.kind}-${item.id}`} href={item.href} className="card block transition hover:border-brand-300"><span className="badge">{item.kind === "concept" ? "Topic" : "Guide"}</span><h2 className="mt-3 text-lg font-semibold">{item.title}</h2><RichContent clampLines={2} className="mt-2 text-sm text-slate-600">{item.summary}</RichContent></Link>) : <p className="text-sm text-slate-500">No topics in this folder yet. Use + Add topic above.</p>}</div>}</div>;
}
