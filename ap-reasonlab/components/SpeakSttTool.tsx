...
/**
 * English speech → text.
 * Live mic = browser Web Speech (en-US), writing as you speak.
...
export default function SpeakSttTool() {
  ...
  const toggleListen = useCallback(() => {
    ...
    setListening(true);
    setStatus("Writing as you speak (en-US)…");
    ...
    const attach = () => {
      ...
      rec.onresult = (event) => {
        ...
        if (finalText.trim()) {
          ...
          // Live interim — replace only the pending phrase (no echo / stack).
          flushSync(() => {
      ...
      rec.onend = () => {
        // Chrome ends after pauses even with continuous=true — restart so text keeps flowing.
    ...
    attach();
  }, [appendFinal, listening, stopListen]);
  ...
  return (
    <StudyToolShell
      ...
      description="English speech → text while you speak. Live mic on Chrome/Edge, or upload / record an audio file (Whisper)."
      tip="Live mic writes into the box as you talk (no wait for a final phrase). Files and recorded clips use Groq Whisper — max 4 MB (mp3, wav, m4a, webm, ogg)."
    >
      <div className="space-y-4">
        ...
        <ul className="list-disc space-y-1 pl-5 text-xs text-slate-500">
          <li>Live mic: browser speech recognition — text streams while you speak (en-US).</li>
          <li>Audio file / record clip: server Whisper transcription (needs site Groq key). Echo cancellation is on for clips.</li>
