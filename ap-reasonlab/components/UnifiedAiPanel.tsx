...
export default function UnifiedAiPanel({
  ...
  async function runLocal(
    ...
    setSiteHits(hits);
    setSiteSearchNote(
      note || (hitCount ? `Using ${hitCount} Liji Explore hit(s).` : "No site matches.")
    );
    const sitePrefer = completeOptions?.sitePrefer || "formulas";
    const siteHint =
      sitePrefer === "language"
        ? "When Liji Explore site materials are appended below, prefer useful English language snippets (vocab, phrases, example sentences) and cite the hit titles. Ignore AP science / formula hits. Follow the same English teaching rules as the cloud tutor."
        : sitePrefer === "code"
          ? "When Liji Explore site materials are appended below, prefer coding playgrounds, snippets, and programming docs. Cite hit titles. Ignore off-topic AP formula sheets. Follow the coding teaching rules."
          : "When Liji Explore site materials are appended below, prefer their formulas/definitions and cite the hit titles. Ignore off-topic hits. Follow the same teaching rules as the cloud teacher for this tool.";
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 bg-slate-50 px-4 py-3 md:px-5">
        ...
        <p className="mt-1 text-sm text-slate-600">
          ...
          <strong>Always search Liji Explore</strong> is on — AI teaches from site materials.
        </p>
        {siteSearchNote ? (
