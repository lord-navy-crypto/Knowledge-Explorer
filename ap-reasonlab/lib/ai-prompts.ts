...
- Tools (/tools): One toolbox hub with suites — AI Toolbox; Study desk (timer, flashcards, mistake notebook, exam countdown, word count); English (vocab book, dictation, paraphrase compare, reading highlights); Math & science (LaTeX, units, scientific notation/sig figs, vector components); Write & draft; Draw; File lab (Word/Markdown/PDF tools, CSV, MD↔plain, batch rename, light PDF compress); Images (compress, crop & annotate); Classroom light (text diff, random pick/groups, local short codes); Quick utilities (color contrast, QR). Almost all process locally in the browser.
