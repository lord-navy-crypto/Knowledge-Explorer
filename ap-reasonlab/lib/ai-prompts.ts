...
Home: four gateway boxes on / ; AI Toolbox, Forum, and Manage stay in the top bar.
Main boxes:
- Box 1 Simulation & Download (/explore/workshops) — Simulation Workshop + Download
...
Top bar (always visible): AI Toolbox (/hints), Forum (/forum), Manage (/manage), Quick search
...
- English (/english): Hub split into three lanes — (1) Exams: TOEFL iBT official task types (Complete the Words, daily/academic reading, four listening tasks, sentence/email/discussion writing, listen-and-repeat + interview) and Digital SAT (short passage + one R&W question; Math domains); (2) Skills for real English: Vocabulary, Grammar & Sentences, plus local practice tools (vocab book, speech-to-text mic/file, dictation, paraphrase, reading highlights); (3) English AI Tutor kept at /english/ai → AI Toolbox English.
- Code (/code): one editor at /code/editor with a language picker. Browser runtimes: Python (Pyodide), JavaScript, TypeScript, Web/HTML, SQL (sql.js), Markdown. Java and C# use Practice Run (teaching subset → JS stand-in; download for real JDK/.NET). C, C++, Go, Rust, PHP, Ruby, R, Swift, Kotlin use Practice Run as a brace/quote check only (not a compiler) + download. Optional real Java runner: PISTON_URL → /api/code/run-java. Hub cards stay at /code/python etc. and redirect into the editor. Long code block adder at /tools/code-board.
